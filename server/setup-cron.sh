...
CRON_MONITOR="0 * * * * /home/ubuntu/lazo-app/server/disk-monitor.sh >> /var/log/disk-monitor.log 2>&1"
CRON_DAILY_CLEANUP="0 3 * * * /usr/bin/docker system prune -a -f --volumes >> /var/log/docker-cleanup.log 2>&1"
CRON_LOG_ROTATION="0 4 * * 0 /usr/bin/journalctl --vacuum-time=14d >> /var/log/log-rotation.log 2>&1"
